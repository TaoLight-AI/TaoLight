...
function finishEarly(){const w=activeWorkout();w.completedAt=new Date().toISOString();w.restUntil=0;if(w.sets.length){w.status="done";w.endedEarly=true;toast("已保留完成量，不需要补课")}else{w.status="cancelled";toast("未产生训练记录，计划仍可重新开始")}persist();render()}
