...
function activeWorkout(){return workouts.find(x=>x.status==="active"&&x.date===today())||null}
...
function suggest(exerciseId,excludeId){return CoachEngine.prescribe({sessions:completed().filter(w=>w.id!==excludeId),exerciseId,startWeight:exerciseSpec(exerciseId)?.startWeight??0,restricted:Object.values(profile.risks||{}).some(Boolean)})}
...
function persist(){save(K.workouts,workouts)}
...
function recordSet(quality,pain=false){const w=activeWorkout();if(!w)return;const e=currentExercise(w),weight=+q("#v5Weight")?.value,reps=+q("#v5Reps")?.value;if(!pain&&(!Number.isFinite(weight)||weight<0||weight>1000||!Number.isInteger(reps)||reps<1||reps>100))return toast("第一次请留下重量和次数，以后会自动沿用");const same=w.sets.filter(s=>s.exerciseIndex===w.currentExercise).length;w.sets.push({exerciseId:e.id,name:e.name,exerciseIndex:w.currentExercise,set:same+1,weight:weight||0,reps:reps||0,quality,pain,rir:q("#coachRir")?.value!==""?Number(q("#coachRir")?.value):null,source:"user",at:new Date().toISOString()});w.draftWeight=null;w.draftReps=null;if(pain){w.status="stopped";w.completedAt=new Date().toISOString();w.restUntil=0;persist();render();toast("训练已停止，安全边界已锁定");queuePhaseReview();return}if(same+1>=e.targetSets){if(w.currentExercise>=w.exercises.length-1){w.status="done";w.completedAt=new Date().toISOString();w.restUntil=0}else{w.currentExercise++;w.restUntil=Date.now()+90e3}}else w.restUntil=Date.now()+90e3;persist();render();if(w.status==="done")queuePhaseReview()}
...
async function checkHuawei(){
 const el=modal("华为运动健康",`<p>授权后可读取获批范围内的步数、睡眠和心率。只有实际收到数据才显示同步成功。</p><div id="healthStatus">正在检测连接…</div><div id="healthActions"></div><p>解除连接会删除服务端华为数据及令牌。华为侧权限可在运动健康的授权管理中撤回。</p>`);
 const status=el.querySelector('#healthStatus'),actions=el.querySelector('#healthActions');
 let session;try{session=JSON.parse(sessionStorage.getItem('coach-health-session')||'null')}catch{}
 const api=async(action,method='GET')=>{const r=await fetch(`${SUPABASE_URL}/functions/v1/huawei-oauth?action=${action}`,{method,headers:{apikey:SUPABASE_KEY,...(session?.access_token?{Authorization:`Bearer ${session.access_token}`}:{})},signal:AbortSignal.timeout(15000)});let data;try{data=await r.json()}catch{throw Error('同步服务暂不可用')}if(!r.ok)throw Error(data.error||'服务暂不可用');return data;};
 try{const result=await api('status');status.textContent=result.connected?`已授权 · ${result.last_synced_at?'上次同步 '+new Date(result.last_synced_at).toLocaleString():'尚未同步数据'}`:'尚未授权';
 actions.innerHTML=`<button type="button" class="v5-primary" data-health-connect>${result.connected?'同步最近7天':'前往华为授权'}</button>${result.connected?'<button type="button" class="v5-secondary" data-health-disconnect>解除连接并删除华为数据</button>':''}`;
 actions.querySelector('[data-health-connect]').onclick=async event=>{event.target.disabled=true;try{const data=await api(result.connected?'sync':'authorize','POST');if(data.authorize_url){const target=new URL(data.authorize_url);if(target.origin!=='https://oauth-login.cloud.huawei.com')throw Error('授权地址验证失败');location.href=target.href;return;}services.huawei={connected:true,syncedAt:data.synced_at,count:data.count};save(K.services,services);save(`coach-huawei-${session.user.id}`,data.samples);status.textContent=`同步成功：收到${data.count}条数据，${new Date(data.synced_at).toLocaleString()}`;}catch(error){status.textContent=error.message}finally{event.target.disabled=false}};
 actions.querySelector('[data-health-disconnect]')?.addEventListener('click',async()=>{if(!confirm('确认解除华为连接并删除已同步的华为数据？训练记录会保留。'))return;try{await api('disconnect','DELETE');localStorage.removeItem(`coach-huawei-${session.user.id}`);services.huawei={connected:false};save(K.services,services);el.remove();render();toast('已解除连接并删除华为数据')}catch(error){status.textContent=error.message}});
 }catch(error){status.textContent=error.message; if(!session){actions.innerHTML=`<form id="healthLogin"><label>同步账户邮箱<input type="email" name="email" autocomplete="username" required></label><label>密码<input type="password" name="password" autocomplete="current-password" required></label><button class="v5-primary" type="submit">登录同步账户</button></form><p>需要已开通的个人同步账户。未开通时仍可手动记录和导出。</p>`;actions.querySelector('form').onsubmit=async event=>{event.preventDefault();const form=new FormData(event.target),button=event.target.querySelector('button');button.disabled=true;try{const r=await fetch(`${SUPABASE_URL}/auth/v1/token?grant_type=password`,{method:'POST',headers:{apikey:SUPABASE_KEY,'Content-Type':'application/json'},body:JSON.stringify({email:form.get('email'),password:form.get('password')}),signal:AbortSignal.timeout(15000)});const data=await r.json();if(!r.ok||!data.access_token)throw Error('账户或密码不正确，或服务未开通');sessionStorage.setItem('coach-health-session',JSON.stringify({access_token:data.access_token,user:{id:data.user.id}}));event.target.reset();el.remove();checkHuawei()}catch(error){status.textContent=error.message}finally{button.disabled=false}};}}
}
...
function undoSet(){
 const w=activeWorkout();if(!w?.sets.length)return toast("没有可撤销的记录");
 const previous=w.sets.pop();w.currentExercise=previous.exerciseIndex;w.draftWeight=previous.weight;w.draftReps=previous.reps;w.restUntil=0;
 persist();render();toast("误记已撤销，请重新确认本组");
}
function coachDataCard(){return `<article class="v5-card coach-data"><small>我的数据 · 可随时带走</small><h2>训练与恢复档案</h2><p>完整导出包含新旧训练、逐组余力、睡眠与已确认餐食。导出文件含健康信息，请只分享给可信任的人。</p><div class="coach-actions"><button type="button" class="v5-secondary" data-training-history>查看训练历史</button><button type="button" class="v5-secondary" data-health-record>记录睡眠与围度</button><button type="button" class="v5-primary" data-export-all>导出完整教练数据</button><label class="v5-secondary">预览导入数据<input type="file" data-import-all accept=".json,application/json"></label></div></article>`}
function exportAll(){
 const health=load(`coach-health-${account}`,[]);
 const records=completed().map(w=>({id:w.id,logDate:w.date,planId:w.planId,sessionType:w.title,status:w.status,endedEarly:!!w.endedEarly,source:w.source||"user",startedAt:w.startedAt,completedAt:w.completedAt,sets:w.sets,exercises:w.exercises,readiness:w.readiness}));
 const data={format:"taolight-coach-data",schema_version:2,exported_at:new Date().toISOString(),timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,profile,records,health,readiness,meals:mealDays,decision_version:CoachEngine.version,privacy:"含本人健康信息；不含账户凭据和照片"};
 const url=URL.createObjectURL(new Blob([JSON.stringify(data,null,2)],{type:"application/json"})),a=document.createElement("a");a.href=url;a.download=`taolight-coach-data-${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast("完整数据包已导出");
}
async function importAll(event){
 const file=event.target.files?.[0];if(!file)return;if(file.size>5e6)return toast("文件超过5MB，请分批导入");
 try{const data=JSON.parse(await file.text()),audit=CoachEngine.validatePackage(data,today());
 const el=modal("核对导入身份",`<p>文件档案：${esc(data.profile?.name||"未提供姓名，请核实来源")}。有效记录${audit.records.length}条，异常或重复${audit.issues.length}条。</p><p>${esc(audit.issues.slice(0,5).join("；"))}</p><p>这里只导入训练记录，原档案、医嘱、餐食及睡眠不会被覆盖。请确认全部属于你本人。</p><button type="button" class="v5-primary" data-confirm-import>确认是本人记录并合并</button>`);
 el.querySelector('[data-confirm-import]').onclick=()=>{const ids=new Set(workouts.map(w=>w.id));let n=0;for(const r of audit.records){if(!Array.isArray(r.sets)||!r.sets.length)continue;const id=r.id||`import-${r.logDate}-${r.planId||r.sessionType}`;if(ids.has(id))continue;ids.add(id);workouts.push({...r,id,date:r.logDate,title:r.sessionType||r.title||"导入训练",planId:r.planId||"imported",status:r.status==='stopped'?'stopped':'done',source:'import',exercises:Array.isArray(r.exercises)?r.exercises:[],completedAt:r.completedAt||r.logDate+'T12:00:00'});n++;}persist();el.remove();render();toast(`已合并${n}条；未覆盖已有记录`)};
 }catch(error){toast(error.message||"导入失败，原记录未改变")}
}
function healthRecord(){
 const fields=[['sleepHours','睡眠小时',0,16,.1],['wakeCount','夜醒次数',0,30,1],['bodyWeight','晨起体重 kg',20,300,.1],['waist','同位置腰围 cm',30,250,.1],['restingHr','静息心率 次/分',25,220,1]];
 const el=modal("睡眠与身体记录",`<form id="coachHealthForm"><label>测量日期<input name="date" type="date" value="${today()}" max="${today()}" required></label>${fields.map(([key,label,min,max,step])=>`<label>${label}<input name="${key}" type="number" min="${min}" max="${max}" step="${step}" placeholder="未测可留空"></label>`).join('')}<p>来源：本人填写。设备测量值请按原单位填写，未测不填0。</p><button type="submit" class="v5-primary">保存本次测量</button></form>`);
 el.querySelector('form').onsubmit=event=>{event.preventDefault();const form=new FormData(event.target),date=form.get('date');if(!CoachEngine.validDate(date)||date>today())return toast('日期无效');const record={id:crypto.randomUUID(),date,source:'user',at:new Date().toISOString()};for(const [key,label,min,max] of fields){const raw=form.get(key);if(raw==='')continue;const v=Number(raw);if(!Number.isFinite(v)||v<min||v>max)return toast(label+'超出范围');record[key]=v;}if(Object.keys(record).length===4)return toast('至少填写一项测量');const rows=load(`coach-health-${account}`,[]);rows.push(record);save(`coach-health-${account}`,rows);el.remove();toast('已保存原始测量，未据单点自动加重');};
}
function trainingHistory(){const rows=completed();modal("训练历史",rows.length?rows.map(w=>`<details class="coach-history"><summary>${esc(w.date)} · ${esc(w.title)} · ${(w.sets||[]).length}组</summary>${(w.sets||[]).map(s=>`<p>${esc(s.name||s.exerciseId)}：${s.weight??'—'}kg × ${s.reps??'—'}次 · RIR ${s.rir??'未记录'}${s.pain?' · 有不适':''}</p>`).join('')}</details>`).join(''):'<p>还没有完成记录。完成训练后可在这里核对每一组。</p>')}

function onboarding(){q(".v5-modal")?.remove();const modal=document.createElement("div");modal.className="v5-modal v5-onboarding-modal";modal.innerHTML=`<form class="v5-sheet v5-onboarding-sheet"><button type="button" class="v5-close">×</button><small>一次建档，以后不重复填写</small><h2>只需30秒，建立专属计划</h2><div class="v5-onboarding-fields"><label>怎样称呼你<input id="v5Name" value="${esc(profile.name||"")}" required placeholder="你的称呼"></label><label>当前体重（用于蛋白质参考）<input id="v5BodyWeight" type="number" min="30" max="250" step="0.1" value="${currentWeight()||""}" placeholder="kg"></label><label>90天唯一目标<select id="v5Goal"><option ${/睡|肌/.test(profile.goal||"")?"selected":""}>改善睡眠并增加肌肉</option><option ${/减脂|塑形/.test(profile.goal||"")?"selected":""}>减脂并改善体型</option><option ${/体能|精力/.test(profile.goal||"")?"selected":""}>提升体能与日常精力</option></select></label></div><fieldset><legend>会改变训练的安全边界</legend><label><input type="checkbox" data-risk="doctor" ${profile.risks?.doctor?"checked":""}>医生限制运动或近期手术</label><label><input type="checkbox" data-risk="heart" ${profile.risks?.heart?"checked":""}>运动时胸痛、晕厥或异常气短</label><label><input type="checkbox" data-risk="eye" ${profile.risks?.eye?"checked":""}>眼部手术、视网膜或视力限制</label><label><input type="checkbox" data-risk="joint" ${profile.risks?.joint?"checked":""}>明显关节、腰背或盆底不适</label></fieldset><div class="v5-onboarding-submit"><label class="v5-consent"><input id="v5Consent" type="checkbox">同意在本机保存必要训练与健康信息</label><button class="v5-primary" type="submit">保存并进入今天</button></div></form>`;document.body.appendChild(modal);q(".v5-close").onclick=()=>profile.goal?modal.remove():toast("完成一次建档后才能生成安全训练");modal.onsubmit=e=>{e.preventDefault();if(!q("#v5Consent").checked)return toast("需要同意后才能建立本机档案");const risks={};qa("[data-risk]").forEach(x=>risks[x.dataset.risk]=x.checked);profile={...profile,name:q("#v5Name").value.trim(),weight:Number(q("#v5BodyWeight").value)||null,goal:q("#v5Goal").value,risks,startedAt:profile.startedAt||new Date().toISOString()};save(K.profile,profile);modal.remove();view="today";render();toast("建档完成，以后直接告诉你下一步")}}
