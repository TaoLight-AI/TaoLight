...
function nutritionHistoryCard(){
  ...
  return`<article class="v5-card v5-nutrition-history"><small>饮食证据 · 最近${days.length}个记录日</small><h2>蛋白质与热量趋势</h2><div class="v5-evidence-row"><section><b>${avgProtein}</b><span>日均蛋白 g</span></section><section><b>${avgCalories}</b><span>日均 kcal</span></section><section><b>${target?`${targetDays}/${days.length}`:"—"}</b><span>蛋白达标日</span></section></div><p>${days.length<3?"记录日还少，先继续随餐拍照，不据此调整长期计划。":target&&targetDays<Math.ceil(days.length*.6)?"蛋白质达标日偏少；下一阶段先提高每餐蛋白，不同时改变训练量。":"当前营养记录已能支持趋势观察；继续确认份量，不追逐单日热量。"}</p></article>`
}
...
function renderMealResult(id,result){
  ...
  return`<section class="v5-meal-result ${result.confirmed?"confirmed":""}"><div class="v5-meal-result-head"><span>${result.confirmed?"已计入全天":"识别结果 · 确认后计入"}</span><em>${confidence}置信度</em></div><label>识别食物<input id="v5MealItems" value="${esc(mealItemsText(result))}" placeholder="例如：米饭、鸡胸、青菜"></label><div class="v5-meal-numbers"><label><span>热量</span><div><input id="v5MealCalories" type="number" min="0" max="5000" inputmode="numeric" value="${Math.round(Number(result.calories)||0)}"><b>kcal</b></div></label><label><span>蛋白质</span><div><input id="v5MealProtein" type="number" min="0" max="300" step="0.1" inputmode="decimal" value="${Number(result.protein)||0}"><b>g</b></div></label></div><small>${esc(range)} · 可根据实际份量直接改数字</small><button class="${result.confirmed?"v5-secondary":"v5-primary"}" data-confirm-meal="${id}">${result.confirmed?"保存本餐修改":"确认计入全天"}</button></section>`
}
...
async function analyzeCurrentMeal(){
  ...
  try{
    const image=file?await compressMealPhoto(file):"",response=await fetch(`${SUPABASE_URL}/functions/v1/analyze-meal`,{method:"POST",headers:{"Content-Type":"application/json","apikey":SUPABASE_KEY,"Authorization":`Bearer ${SUPABASE_KEY}`},body:JSON.stringify({image,meal_type:activeMeal,context})}),raw=await response.text();let result={};try{result=JSON.parse(raw)}catch{}if(!response.ok||result.error)throw Error(result.error||(response.status===404?"餐食识别服务尚未部署":"识别服务暂时不可用"));if(!Array.isArray(result.items)||!result.items.length||!(Number(result.estimated_calories)>0))throw Error("没有识别出有效食物，请补充份量后重试");
    const current=mealsFor();current[activeMeal]={items:result.items,itemsText:mealItemsText(result),calories:Number(result.estimated_calories)||0,protein:Number(result.protein_g)||0,calorieMin:Number(result.calorie_min)||0,calorieMax:Number(result.calorie_max)||0,confidence:result.confidence||"low",assumptions:result.assumptions||"",confirmed:false,source:"ai",photoStored:false,analyzedAt:new Date().toISOString()};mealDays[today()]=current;saveMeals();render();setTimeout(()=>q(".v5-meal-result")?.scrollIntoView({behavior:"smooth",block:"center"}),0);toast("识别完成，请看一眼并确认")
