const CACHE="private-ai-coach-20260907-3";
const ASSETS=["./","./index.html","./steward-v5.css?v=20260904-2","./nutrition-v7.css?v=20260903-4","./steward-v5.js?v=20260907-3","./coach-engine.js?v=20260907-3","./coach-upgrade.css?v=20260907-3","./app-icon-512.png","./manifest.webmanifest"];
...
self.addEventListener("activate",event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key!==CACHE&&(key.startsWith("steward-")||key.startsWith("private-ai-coach-"))).map(key=>caches.delete(key)))).then(()=>self.clients.claim()))});
self.addEventListener("fetch",event=>{
  const url=new URL(event.request.url);
  if(event.request.method!=="GET"||url.origin!==self.location.origin||url.searchParams.has("action")||event.request.headers.has("authorization"))return;
  const known=ASSETS.some(path=>new URL(path,self.registration.scope).href===url.href);
  ...
  }).catch(async()=>await caches.match(event.request)||(event.request.mode==="navigate"?await caches.match("./index.html"):undefined)||Response.error()));
});
...
self.addEventListener("push",event=>{const data=event.data?.json?.()||{};event.waitUntil(self.registration.showNotification(data.title||"专属私人AI教练",{body:data.body||"拥有自律，生活治愈。今天的训练已经准备好。",icon:"./app-icon-512.png",badge:"./app-icon-512.png",tag:data.tag||"steward-push",data:{url:data.url||"./"}}))});
