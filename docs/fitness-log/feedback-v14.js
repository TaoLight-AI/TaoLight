...
(function(){
  ...
  function dailyModel(p){
    ...
    let next="训练已结束，不再追加运动；分次补水、正常吃饭，并尽量在固定时间上床。";
    if(reason)next="先停止运动并确认异常；若症状正在发生、反复或加重，请及时就医。";
    else if(tone==="warn")next="今晚优先恢复：不补训、不饮酒、减少临睡前大量饮水，并把上床时间提前30分钟。";
    else if(duration&&completion>=85&&rpe<=8)next="今天的刺激已经够用；接下来只做恢复，不用再用额外运动证明完成度。";
    else if(!duration)next="今天按恢复日处理，不用补课；完成轻松活动、正常进餐和按时睡觉即可。";
  ...
  function coachText(a){return`${a.headline}。${a.priority}\n\n训练结论：${a.achievement} ${a.training_analysis}\n\n恢复与饮食：${a.nutrition_analysis} ${a.recovery_analysis}\n\n接下来24小时：${a.tomorrow_plan}`}
  function applyModelCoach(root,a){if(!root||!a)return;root.querySelector(".v14-summary h3")?.replaceChildren(a.headline);root.querySelector(".v14-summary p")?.replaceChildren(a.priority);root.querySelector(".v14-status")?.replaceChildren(a.status);const cards=root.querySelectorAll(".v14-actions article p");if(cards[0])cards[0].textContent=a.achievement;if(cards[1])cards[1].textContent=`${a.nutrition_analysis} ${a.recovery_analysis}`;if(cards[2])cards[2].textContent=a.tomorrow_plan;const evidence=root.querySelector(".v14-evidence p");if(evidence&&a.evidence?.length)evidence.textContent=`本次教练判断依据：${a.evidence.join("；")}。模型置信度：${a.confidence}。评分指标用于趋势管理，不是医学诊断。`;currentFeedback=coachText(a);root.dataset.coachSource="model"}
